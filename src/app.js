...
const APP = (() => {
        ...
        <div class="node-wrapper" style="left: ${pos.x}%; top: ${pos.y}%;">
          <div class="node-content-animator">
            <div class="resolution-node interactive-surface" style="--node-color: ${res.color};"
                 ...
                 data-context-id="home.mindmap.node.${res.id}"
                 data-resolution-id="${res.id}"
                 data-section-id="mindmap"
                 data-section-type="mindmap-node">
  ...
  function generateMobileMindMapHTML() {
    return RESOLUTIONS_DATA.map(res => `
      <div class="resolution-card interactive-surface"
           ...
           onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();APP.navigateTo('#/nghi-quyet/${res.id}');}"
           data-ai-context="true"
           data-context-id="home.mindmap.card.${res.id}"
           data-resolution-id="${res.id}"
           data-section-id="mindmap"
           data-section-type="mindmap-card">
  ...
  function showDetailPage(id) {
    ...
    // Tự động kích hoạt Trợ lý AI tóm tắt khi click infographic
    if (window._lastClickContext && window._lastClickContext.type === 'node') {
      if (window.AICopilot) {
        setTimeout(() => {
          window.AICopilot.startInteraction();
          window.AICopilot.setUIState('PROCESSING');
          setTimeout(() => {
            window.AICopilot.playResponse(`Tóm tắt Nghị quyết ${res.id}`);
          }, 100);
        }, 1200); // Đợi hiệu ứng chuyển trang hoàn tất
      }
    }
      ...
      introHTML = `
        <div class="content-section reveal-on-scroll">
          <div class="resolution-intro theme-on-red content-backplate" style="font-size:1.25rem; line-height:1.9; font-weight:500; border-left: 4px solid ${res.color}; padding-left: 20px; margin-bottom: 30px; background: var(--glass-bg); padding: 20px; border-radius: var(--radius-md); box-shadow: var(--shadow-sm); backdrop-filter: blur(14px); -webkit-backdrop-filter: blur(14px); border: 1px solid rgba(255,255,255,0.1);"
               data-ai-context="true"
               data-context-id="nq${res.id}.intro"
               data-resolution-id="${res.id}"
               data-section-id="intro"
               data-section-type="intro"
               data-item-index="0"
               data-context-title="Giới thiệu"
               data-context-summary="Nội dung giới thiệu Nghị quyết ${res.id}"
               data-source-anchor="Tài liệu NQ ${res.id}.pdf"
               data-context-type="intro"
               data-related-context="nq${res.id}.intro">
            ${res.intro}
          </div>
        </div>
      ...
      contextHTML = `
        <div class="content-section reveal-on-scroll">
          <div class="glass-context-box theme-on-red content-backplate" style="--card-color: ${res.color}; --text-color: ${getContrastColor(res.color)};"
               data-ai-context="true"
               data-context-id="nq${res.id}.context"
               data-resolution-id="${res.id}"
               data-section-id="context"
               data-section-type="context"
               data-item-index="0"
               data-context-title="Bối cảnh ra đời"
               data-context-summary="Bối cảnh ra đời của Nghị quyết ${res.id}"
               data-source-anchor="Tài liệu NQ ${res.id}.pdf"
               data-context-type="context"
               data-related-context="nq${res.id}.context">
          ...
          <div class="glass-summary-grid balanced-grid" data-count="${res.coreIdeas.length}">
            ${res.coreIdeas.map((idea, i) => `
              <div class="glass-summary-card reveal-on-scroll" style="--card-color: ${res.color}; --text-color: ${getContrastColor(res.color)}; transition-delay: ${i*50}ms;"
                   data-ai-context="true"
                   data-context-id="nq${res.id}.core-ideas.${i+1}"
                   data-resolution-id="${res.id}"
                   data-section-id="coreIdeas"
                   data-section-type="core-ideas"
                   data-item-index="${i}"
                   data-context-title="Tinh thần cốt lõi"
                   data-context-summary="${idea.replace(/"/g, '&quot;')}"
                   data-source-anchor="Tài liệu NQ ${res.id}.pdf"
                   data-context-type="core-idea"
                   data-related-context="nq${res.id}.core-ideas.${i+1}">
          ...
          <div class="glass-pillar-grid balanced-grid" data-count="${res.generalGoals.length}">
            ${res.generalGoals.map((b, i) => `
              <div class="glass-pillar-card reveal-on-scroll" style="--card-color: ${res.color}; --text-color: ${getContrastColor(res.color)}; transition-delay: ${i*80}ms;"
                   data-ai-context="true"
                   data-context-id="nq${res.id}.general-goals.${i+1}"
                   data-resolution-id="${res.id}"
                   data-section-id="generalGoals"
                   data-section-type="general-goals"
                   data-item-index="${i}"
                   data-context-title="${b.title.replace(/"/g, '&quot;')}"
                   data-context-summary="${b.desc.replace(/"/g, '&quot;')}"
                   data-source-anchor="Tài liệu NQ ${res.id}.pdf"
                   data-context-type="general-goal"
                   data-related-context="nq${res.id}.general-goals.${i+1}">
        ...
        <div class="content-section reveal-on-scroll">
          ...
          <div class="glass-pillar-grid balanced-grid" data-count="${res.infographicBlocks.length}">
            ${res.infographicBlocks.map((b, i) => `
              <div class="glass-pillar-card reveal-on-scroll" style="--card-color: ${res.color}; --text-color: ${getContrastColor(res.color)}; transition-delay: ${i*80}ms;"
                   data-ai-context="true"
                   data-context-id="nq${res.id}.infographic-blocks.${i+1}"
                   data-resolution-id="${res.id}"
                   data-section-id="infographicBlocks"
                   data-section-type="infographic-blocks"
                   data-item-index="${i}"
                   data-context-title="${b.title.replace(/"/g, '&quot;')}"
                   data-context-summary="${b.desc.replace(/"/g, '&quot;')}"
                   data-source-anchor="Tài liệu NQ ${res.id}.pdf"
                   data-context-type="infographic-block"
                   data-related-context="nq${res.id}.infographic-blocks.${i+1}">
      ...
      if (hasPercentages && res.id === "57") { // Keep radial for 57 to ensure we don't break logic, but we can do it for any
        innerHTML = `<div class="glass-radial-grid balanced-grid" data-count="${res.goals2030.length}">
          ${res.goals2030.map((g, i) => {
            ...
            const valLen = g.value.length;
            const sizeClass = valLen > 8 ? 'text-long' : (valLen > 5 ? 'text-medium' : '');
            return `
              <div class="glass-radial-card reveal-on-scroll" style="--card-color: ${res.color}; --text-color: ${getContrastColor(res.color)}; transition-delay: ${i*50}ms;"
                   data-ai-context="true"
                   data-context-id="nq${res.id}.goals-2030.${i+1}"
                   data-resolution-id="${res.id}"
                   data-section-id="goals2030"
                   data-section-type="goals-2030"
                   data-item-index="${i}"
                   data-context-title="${g.label.replace(/"/g, '&quot;')}"
                   data-context-summary="${g.value} - ${g.description.replace(/"/g, '&quot;')}"
                   data-source-anchor="Tài liệu NQ ${res.id}.pdf"
                   data-context-type="stat"
                   data-related-context="nq${res.id}.goals-2030.${i+1}">
                <div class="glass-radial-circle">
                  ...
                  <div class="glass-radial-value auto-counter ${sizeClass}" data-raw-text="${g.value}">${g.value}</div>
                </div>
          ...
          ${res.goals2030.map((g, i) => {
            ...
            return `
              <div class="glass-stat-card reveal-on-scroll" style="--card-color: ${res.color}; --text-color: ${getContrastColor(res.color)}; transition-delay: ${i*60}ms;"
                   ...
                   data-context-type="stat"
                   data-related-context="nq${res.id}.goals-2030.${i+1}">
                <div class="glass-stat-value auto-counter ${sizeClass}" data-raw-text="${g.value}">${g.value}</div>
          ...
          <div class="glass-timeline">
            ${res.vision2045.map((v, i) => `
              <div class="glass-timeline-item reveal-on-scroll" style="--card-color: ${res.color}; --text-color: ${getContrastColor(res.color)}; transition-delay: ${i*70}ms;"
                   data-ai-context="true"
                   data-context-id="nq${res.id}.vision-2045.${i+1}"
                   data-resolution-id="${res.id}"
                   data-section-id="vision2045"
                   data-section-type="vision-2045"
                   data-item-index="${i}"
                   data-context-title="Tầm nhìn 2045"
                   data-context-summary="${v.replace(/"/g, '&quot;')}"
                   data-source-anchor="Tài liệu NQ ${res.id}.pdf"
                   data-context-type="timeline"
                   data-related-context="nq${res.id}.vision-2045.${i+1}">
              ...
              if (typeof t === 'string') {
                return `
                  <div class="glass-timeline-item reveal-on-scroll" style="--card-color: ${res.color}; --text-color: ${getContrastColor(res.color)}; transition-delay: ${(i%5)*60}ms;"
                       data-ai-context="true"
                       data-context-id="nq${res.id}.tasks.${i+1}"
                       data-resolution-id="${res.id}"
                       data-section-id="tasks"
                       data-section-type="tasks"
                       data-item-index="${i}"
                       data-context-title="Nhiệm vụ và giải pháp chủ yếu"
                       data-context-summary="${t.replace(/"/g, '&quot;')}"
                       data-source-anchor="Tài liệu NQ ${res.id}.pdf"
                       data-context-type="timeline"
                       data-related-context="nq${res.id}.tasks.${i+1}">
                    <div class="glass-timeline-dot"></div>
                    <div class="glass-timeline-content">
                ...
                return `
                  <div class="glass-timeline-item reveal-on-scroll" style="--card-color: ${res.color}; --text-color: ${getContrastColor(res.color)}; transition-delay: ${(i%5)*60}ms;"
                       ...
                       data-section-type="tasks"
                       data-item-index="${i}"
                       data-context-title="${t.title.replace(/"/g, '&quot;')}"
                       data-context-summary="${t.description.replace(/"/g, '&quot;')}"
                       data-source-anchor="Tài liệu NQ ${res.id}.pdf"
                       data-context-type="timeline"
                       data-related-context="nq${res.id}.tasks.${i+1}">
                    <div class="glass-timeline-dot"></div>
          ...
          <div class="action-grid">
            ${res.newPoints.map((point, i) => `
              <div class="action-card readable-glass-card theme-on-red reveal-on-scroll reveal-delay-${i}"
                   ...
                   data-resolution-id="${res.id}"
                   data-section-id="newPoints"
                   data-section-type="new-points"
                   data-item-index="${i}"
                   data-context-title="Điểm mới đột phá"
                   data-context-summary="${point.replace(/"/g, '&quot;')}"
                   data-source-anchor="Tài liệu NQ ${res.id}.pdf"
          ...
          <div class="action-grid-5">
            ${res.responsibilities.map((resp, i) => `
              <div class="action-card readable-glass-card content-backplate reveal-on-scroll reveal-delay-${i}" style="border-top-color: var(--accent-on-red);"
                   data-ai-context="true"
                   data-context-id="nq${res.id}.responsibilities.${i+1}"
                   data-resolution-id="${res.id}"
                   data-section-id="responsibilities"
                   data-section-type="responsibilities"
                   data-item-index="${i}"
                   data-context-title="${resp.title.replace(/"/g, '&quot;')}"
                   data-context-summary="${resp.content.replace(/"/g, '&quot;')}"
                   data-source-anchor="Tài liệu NQ ${res.id}.pdf"
                   data-context-type="card"
                   data-related-context="nq${res.id}.responsibilities.${i+1}">
          ...
          <div>
            ${res.quoteBlocks.map((q, i) => `
              <div class="glass-impact-card readable-glass-card reveal-on-scroll" style="--card-color: ${res.color}; --text-color: ${getContrastColor(res.color)}; transition-delay: ${i*50}ms;"
                   data-ai-context="true"
                   data-context-id="nq${res.id}.quotes.${i+1}"
                   data-resolution-id="${res.id}"
                   data-section-id="quoteBlocks"
                   data-section-type="quotes"
                   data-item-index="${i}"
                   data-context-title="${q.speaker.replace(/"/g, '&quot;')}"
                   data-context-summary="${q.quote.replace(/"/g, '&quot;')}"
                   data-source-anchor="Tài liệu NQ ${res.id}.pdf"
                   data-context-type="quote"
                   data-related-context="nq${res.id}.quotes.${i+1}">
        ...
        <figure class="inline-figure reveal-on-scroll" style="--figure-color:${res.color};">
          <div class="inline-figure-img-wrap" ${res.id === '80' ? 'style="max-height: none;"' : ''}>
            <img src="${img.src}" alt="${img.caption}" class="inline-figure-img"
                 ...
                 data-ai-context="true"
                 data-context-id="nq${res.id}.image.${afterSection}.${galleryIdx >= 0 ? galleryIdx : 0}"
                 data-resolution-id="${res.id}"
                 data-section-id="${afterSection}"
                 data-section-type="image"
                 data-context-summary="${img.caption}">
        ...
        <div class="gallery-strip">
          ${res.gallery.map((g, i) => `
            <figure class="gallery-item interactive-surface" onclick="APP.openLightbox('${allImgsJson}', ${i})"
                    data-ai-context="true"
                    data-context-id="nq${res.id}.gallery.${i}"
                    data-resolution-id="${res.id}"
                    data-section-id="gallery"
                    data-section-type="gallery-item"
                    data-item-index="${i}"
                    data-context-title="Thư viện ảnh"
                    data-context-summary="${g.caption.replace(/"/g, '&quot;')}"
                    data-source-anchor="Tài liệu NQ ${res.id}.pdf"
                    data-context-type="gallery-item"
                    data-related-context="nq${res.id}.gallery.img.${i}">
              <img src="${g.src}" alt="${g.caption}" class="gallery-thumb"
                   loading="lazy" onerror="this.closest('figure').style.display='none'"
                   data-ai-context="true"
                   data-context-id="nq${res.id}.gallery.img.${i}"
                   data-resolution-id="${res.id}"
                   data-section-id="gallery"
                   data-section-type="image"
                   data-item-index="${i}"
                   data-context-title="Hình ảnh thư viện"
                   data-context-summary="${g.caption.replace(/"/g, '&quot;')}"
                   data-source-anchor="Tài liệu NQ ${res.id}.pdf"
                   data-context-type="image"
                   data-related-context="nq${res.id}.gallery.${i}">
